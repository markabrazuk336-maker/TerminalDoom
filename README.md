...
| `Shift` | бег |
| `E`, `Ctrl` или `F` | огонь |
| `PgUp` `PgDn` | смотреть вверх и вниз, `Home` — выровнять |
| `Пробел` | открыть дверь, нажать переключатель |
| `1`…`7` | выбор оружия, `Z` / `X` — перебор |
| `Tab` или `M` | автокарта |
| `N` | следующая карта, `R` — начать заново |
| `G` | сквозь стены |
| `K` | включить и выключить мышь |
| `Esc` | выход |

Мышь включается ключом `--mouse`: движение крутит камеру и голову, левая кнопка
стреляет, правая открывает двери. Курсор захватывается, только пока окно
терминала активно, так что переключение на другое окно ничего не ломает.
...
- **Драки между монстрами** — задетый чужим фаерболом переключается на обидчика
  и дерётся с ним, пока тот не умрёт. Архвайл поднимает трупы обратно на ноги.
...
- **Предметы** — аптечки, бонусы, броня, сферы, патроны, рюкзак, ключи, берсерк,
  костюм, неуязвимость.
- **Двери, лифты, полы, лестницы, раздавливающие потолки, «пончики»,
  телепорты, двери под ключ, выход с уровня** и экран итогов:
  убито / предметы / тайники / время.
...
- **Звук** — свой микшер: лампы `DS*` распаковываются в 16-битный PCM и мешаются
  в один поток, который идёт прямо в `waveOut`, поэтому выстрел не обрывает
  рычание монстра. На Linux и macOS звуки уходят системному проигрывателю
  (`paplay`, `aplay`, `afplay`). Отключается `--nosound`.
- **Музыка** — лампы `D_*`: MIDI играется системным синтезатором, а формат MUS
  из оригинального `doom.wad` сначала переводится в MIDI своим конвертером.
  Трек зациклен, отключается `--nomusic`.
- **Прицел по вертикали** — `PgUp`/`PgDn` или мышь; выстрелы и снаряды летят
  туда, куда смотришь.
...
| [dm/sound.py](dm/sound.py) | микшер на `waveOut`, запасной вывод, музыка через MCI |
| [dm/mus.py](dm/mus.py) | конвертер MUS → MIDI |
...
python tools/specials_test.py                    # лестницы, прессы, телепорт, архвайл
...
- нет сохранений, записи и проигрывания демок, сетевой игры;
- расширения BOOM и формат карт UDMF не поддерживаются — только классический
  формат DOOM;
- монстры не открывают двери сами;
- настоящий микшер только на Windows: на Linux и macOS каждый звук запускает
  системный проигрыватель, а для музыки нужен установленный `timidity` или
  `fluidsynth`;
- отсечение спрайтов приблизительное: рисуются по окну колонки, а не по
  точному силуэту ближней геометрии.
